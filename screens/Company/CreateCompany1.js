    ...
    submitInfo = ()=>{
        this.props.form.validateFields({ force: true }, (error,value) => {//校验并获取一组输入域的值
            if(error){
                ...
                const isThreetoone = this.state.isThreetoone;
                let company = value;
                company.createBy = createBy;
                company.isThreetoone = isThreetoone;
                company.isStop = 1;
                // alert(JSON.stringify(company))
                // return
                cleanParams(company);
                request('/company/addCompany',{
                    method:'POST',
                    body:{
                        "company": company,
                        "userId": userId
                    ...
                    if(res.code === 1){
                        this.props.navigation.navigate("CreateCompany2",res.data);
                    }else{
